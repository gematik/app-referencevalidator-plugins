...
@CommandLine.Command(
    name = "build",
    description =
        "The PluginBuilder helps to create custom validation modules (plugins) for the gematik ReferenceValidator (cf. https://github.com/gematik/app-referencevalidator-plugins)")
@Slf4j
public class PluginBuildCommand implements Runnable {
  ...
  @CommandLine.Parameters(
      paramLabel = "PLUGIN_DEFINITION_DIRECTORY",
      description = "Directory with plugin definition file and supporting resources")
  private String pluginDefinitionDirectoryPath;

  @CommandLine.Option(
      names = {"-t", "--targetFolderPath"},
      description =
          "Output directory for created plugins (default: parent directory of PLUGIN_DEFINITION_DIRECTORY)")
  private String targetFolderPath;

  @CommandLine.Option(
      names = {"-url", "--packageServerUrl"},
      description = "The URL of a FHIR package server to use")
  private String packageServerUrl;

  @CommandLine.Option(
      names = {"-dse", "--disableSuccessAndErrorCodes"},
      description = "Disables success/error codes upon finish (0 and 1)")
  ...
  public void run() {
    try {
      targetFolderPath =
          !StringUtils.isEmpty(targetFolderPath)
              ? targetFolderPath
  ...
  private PluginBuilder setUpPluginBuilder() {
    ...
    return new PluginBuilder(fhirPackageDownloader, pluginZipper);
  }
}
