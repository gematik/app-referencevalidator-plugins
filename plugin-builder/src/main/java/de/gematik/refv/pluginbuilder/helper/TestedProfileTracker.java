...
public class TestedProfileTracker {
  ...
  public TestedProfileTracker(
      String packageFolderPath, Collection<FhirPackage> validationFhirPackages) {
  ...
  /**
   * Tracks the profiles associated with the provided resource and updates the lists of tested
   * profiles accordingly. If the expected validity is true, the profiles are added to the list of
   * tested valid plugin profile URLs, otherwise, they are added to the list of tested invalid
   * plugin profile URLs.
   *
  ...
  public void trackProfiles(IBaseResource resource, boolean expectedValidity) {
    ...
    List<String> profilesAsString =
        profiles.stream().map(this::getProfileValueAsString).collect(Collectors.toList());

    if (!profilesAsString.isEmpty()) {
      if (expectedValidity) validProfileUrlsFoundInTestFiles.addAll(profilesAsString);
      if (!expectedValidity) invalidProfileUrlsFoundInTestFiles.addAll(profilesAsString);
    }
  }
  ...
  public void logMissingTestCaseExamples() throws IOException {
    if (packageFolderPath != null) {
      ...
      allProfileUrls =
          validationFhirPackages.stream()
              .flatMap(
                  p -> {
                    try {
                      return ProfileUrlExtractor.getAllPluginProfileUrls(packageFolderPath, p)
                          .stream();
                    } catch (IOException e) {
      ...
      allProfileUrls =
          ProfileUrlExtractor.getAllPluginProfileUrlsFromInputStream(pluginInputStream);
    }
    ...
    if (!missingProfilesValid.isEmpty() || !missingProfilesInvalid.isEmpty()) {
      log.warn(
          "Some of the profiles in your plugin have no corresponding test-files (valid missing: {} / {} | invalid missing: {} / {}):",
          missingProfilesValid.size(),
          allProfileUrls.size(),
          missingProfilesInvalid.size(),
          allProfileUrls.size());
  ...
  private List<String> getMissingProfiles(boolean validity) {
    ...
    List<String> testedProfiles =
        validity ? validProfileUrlsFoundInTestFiles : invalidProfileUrlsFoundInTestFiles;
