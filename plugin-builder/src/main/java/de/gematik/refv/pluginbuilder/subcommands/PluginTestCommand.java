...
public class PluginTestCommand implements Runnable {
  ...
  @CommandLine.Parameters(
      index = "0",
      paramLabel = "PLUGIN_FILE",
      description = "Path to the plugin file (.zip)")
  private File pluginToBeTested;

  @CommandLine.Parameters(
      index = "1",
      paramLabel = "TEST_FILES_DIRECTORY",
      description = "Path to the directory containing test files")
  ...
  public void run() {
    ...
    if (pluginToBeTested.isDirectory()
        || !pluginToBeTested.getName().toLowerCase().endsWith(".zip")) {
      log.error(
          "The plugin {} you passed is not in the correct file format. Accepted formats: [zip]",
          pluginToBeTested.getAbsolutePath());
      return;
    }
    ...
    try (InputStream pluginToBeTestedAsInputStream = new FileInputStream(pluginToBeTested)) {
      ...
      if (!isTestedSuccessful) {
        log.error(
            "Testing of plugin '{}' was unsuccessful! See more info above.",
            pluginToBeTested.getName());
      }
    } catch (Exception e) {
